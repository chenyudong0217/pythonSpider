...
info_card_task_queue = queue.Queue()
...
def parse_law_card(law_id,content):
    spiderFalg = True
    try:
        result = json.loads(content)
        if result['code'] == 200:
            data = result['data']
            if 'RevisedBasis' in data:
                RevisedBasis = data['RevisedBasis']
                if str(RevisedBasis).__contains__('\'') and str(RevisedBasis).__contains__('<') :
                    soup = BeautifulSoup(RevisedBasis,'html.parser')
                    RevisedBasis = soup.get_text()
                    data['RevisedBasis'] = RevisedBasis
            if 'FailureBasis' in data:
                FailureBasis = data['FailureBasis']
                if str(FailureBasis).__contains__('\'') and str(FailureBasis).__contains__('<'):
                    soup = BeautifulSoup(FailureBasis,'html.parser')
                    FailureBasis = soup.get_text()
                    data['FailureBasis'] = FailureBasis
            db_func.save_law_card(law_id,json.dumps(data,ensure_ascii=False))
        else:
            spiderFalg = False
    except Exception as e:
        print(e)
...
##解析m站 法条详情
def parse_info_m(law_id,content):
    spiderFlag = True
    try:
        result = json.loads(content)
        if result['code'] == 200:
            ...
            db_func.save_law_info_m(law_id,title,content)
        else:
            spiderFlag = False
    except Exception as e:
        print(e)
...
#下载www站 法条详情页html
def download_info(task):
    law_id = task['law_id']
    url = 'https://www.pkulaw.com/chl/'+law_id+'.html?way=listView'
    payload = {}
    headers = {
    ...
    try:
        ...
        if response.status_code == 401 or response.status_code == 567: #401token失效， 567出口ip被限制
            #返回详情任务队列
            info_task_queue.put(task)
        return response.text
    except Exception as e:
        print(e)

def download_law_info_m(task,cookie):
    try:
        url = 'https://m6.pkulaw.com/fulltext/fabao/api/mobile-server/platform-fabao/6.0.0.0.0/article/articlewithoutfblx?columnId='+task['column_id']+'&gidEncoded='+task['law_id']
        payload = {}
        headers = {
            'Host': 'm6.pkulaw.com',
        ...
        while int(time.time()) > outDateTs:
            ...
            outDateTs = proxyIp['outDateTs']
        proxy = {'http':str(proxyIp['address']),'https':str(proxyIp['address'])}
        response = requests.request("GET", url, headers=headers, data=payload, timeout=(10,10),proxies=proxy)
        if response.status_code == 401 or response.status_code == 567: #401token失效， 567出口ip被限制
           #返回详情任务队列
            info_task_queue.put(task)
        return response.text
...
#请求法条标签卡片
def download_law_card(task):
    try:
        url = 'https://m6.pkulaw.com/fulltext/fabao/api/mobile-server/platform-fabao/6.0.0.0.0/article/fields?columnId='+task['column_id']+'&gidEncoded='+task['law_id']
        payload = {}
        headers = {
            'Host': 'm6.pkulaw.com',
...
def downloader():
    while 1:
        try:
            task = None
            if info_card_task_queue.qsize()>0:
                task = info_card_task_queue.get()
                law_id = task['law_id']
                print('spider law card,  id:',law_id)
                content = download_law_card(task)
                if not parse_law_card(law_id,content):
                    info_card_task_queue.put(task)
                time.sleep(0.5)
            elif info_task_queue.qsize()>0:
                task = info_task_queue.get()
                law_id = task['law_id']
                print('spider law content, id:',law_id)
                content = download_info(task)
                info = parse_info(content)
                db_func.update_law_info(info)
                #content = download_law_info_m(task,cookie_service.get_one_vip_cookie())
                #if parse_info_m(law_id,content):
...
def spider_info():
    while 1:
        try:
            if info_task_queue.qsize() >= 60:
                time.sleep(2)
                continue
            laws = db_func.get_need_crawl_laws()
            for law in laws:
                print(law)
                info_card_task_queue.put(law)
